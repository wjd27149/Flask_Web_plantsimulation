...
class Population:
    ...
    def caculate_score(self):
        ...
        for i in range(len(NDSet)):
            ...
            for j in range(len(NDSet[i])):
                ...
                NDSet[i][j].score = NDSet[i][j].score + self.args.p_NDscore * score_ND          #求出非支配解 并且按照同一解集中 cmax的大小排序 然后赋分数
                score_ND += 1
        ...
        for index,pop in enumerate(self.population):
            pop.score += (1 - self.args.p_NDscore) * score_FI
            score_FI += 1
    ...
    # 计策：先全部变异 再 全部两两一起交叉
    def operate(self, generation: int, gear: int):
        if(gear == 0 or gear == 1):
            ...
            self.pm = self.args.pm
            self.pc = self.args.pc       
...
class Algorithm:
    ...
    def chs_setup(self):
        self.ms_list_length = []
        self.J_site = []
        for i in range(self.args.n):             # 第i个工件 
            for j in range(self.args.total_ni[i]):     # 第j个工序
                self.ms_list_length.append(len(self.args.MT[i][j]))  # 用于表示机器选择的列表。每个元素表示相应机器的候选数量。
        ...
        for i in range(self.args.n):
            abs = [i] * self.args.ni[temp]
            self.initial_os_list.extend(abs)
            if(i == sum(self.args.job_num_list[: temp+1]) - 1):
                temp += 1
    ...
    def divide_population(self, gear: int):
        ...
        good_offspring_pop = self.good_population.operate(generation= self.cur_gene, gear= gear)
        bad_offspring_pop = self.bad_population.operate(generation= self.cur_gene, gear= gear)
    ...
    def divide_population_1(self, gear: int):
        self.total_population = Population(self.Undivided_population.population, self.args, self.J_site)

        self.total_population.update_population()
        self.total_population.clone_chs()

        total_offspring_pop = self.total_population.operate(generation= self.cur_gene, gear= gear)

        merged_population = Population(population_= total_offspring_pop, args= self.args, J_site= self.J_site)
        merged_population.caculate_score()
...
if __name__ == "__main__":
    ...
    print(lists_best_GOAL)
    print(lists_best_TIME)
